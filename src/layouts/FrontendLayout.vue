<script setup lang="ts">
import { onBeforeUnmount, onMounted, onUnmounted, ref, watch } from "vue";
import { useLibrary } from "@/stores/library";
import { useScrollLock, useStorage } from "@vueuse/core";
import LibraryButton from "@/components/frontend/LibraryButton.vue";
import { useSettings } from "@/stores/settings";
import { useFrontend } from "@/stores/frontend";

const emits = defineEmits<{
    (e: 'onDataIsReady'): void,
    (e: 'isMobile', value: boolean): void,
}>();

const isOrientationLandscape = ref(true);
const matchMediaOrientation = window.matchMedia("(orientation: portrait) and (max-width: 700px)");
const matchMediaMobile = window.matchMedia("(max-width: 1279px)");

const cookiesAccepted = useStorage('isCookiesAccepted', false)
const isLocked = useScrollLock(document.documentElement);
if (cookiesAccepted.value === false) {
    isLocked.value = true;
}

const settings = useSettings();
const stageStore = useLibrary();
const frontend = useFrontend();

const onMobileSwitch = (e: MediaQueryListEvent) => {
    frontend.isMobile = e.matches;
}

watch(cookiesAccepted, (newValue) => {
    if (newValue === true) {
        isLocked.value = false;
    }
})

onMounted( async () => {

    if (! stageStore.dataIsReady) {

        const responses = await Promise.all([
            settings.fetch(),
            stageStore.getChoirs(),
            stageStore.getAttributes(),
            stageStore.getVideos()
        ]);

        const errors = responses.find(response => response.error);

        if (errors) {
            return;
        }

        await stageStore.videos.forEach((video) => {
            video.choir_name = stageStore.getChoirByID(video.choir_id)?.name ?? '';
        })

        stageStore.dataReady();

        frontend.randomizePersonalVideoCompilation();
    }

    emits('onDataIsReady');

    if(matchMediaOrientation.matches) {
        isOrientationLandscape.value = false;
    } else {
        // Landscape orientation
    }

    matchMediaOrientation.addEventListener('change', onChangeOrientation);
    matchMediaMobile.addEventListener('change', onMobileSwitch);
    frontend.isMobile = matchMediaMobile.matches;
});

onBeforeUnmount(() => {
    matchMediaOrientation.removeEventListener('change', onChangeOrientation);
    matchMediaMobile.removeEventListener('change', onMobileSwitch);
});

const onChangeOrientation = (e: MediaQueryListEvent) => {
    isOrientationLandscape.value = !e.matches;
}

const acceptCookies = () => {
    cookiesAccepted.value = true;
}

</script>

<template>
    <div>
        <slot />
        <div v-if="! cookiesAccepted">
            <div class="fixed top-0 left-0 w-full bg-black bg-opacity-60 h-full z-[20] flex items-end py-5">
                <div class="max-w-7xl px-10 mx-auto overflow-auto max-h-full">
                    <div class="bg-white py-10 px-12 xl:px-20">
                        <div class="grid grid-cols-[150px_auto] xl:grid-cols-[200px_auto] gap-10">
                            <div class="">
                                <img class="" src="/images/mix/cookies.png" alt="">
                            </div>
                            <div class="border-l pl-10">
                                <div class="whitespace-pre-wrap leading-7 text-sm md:text-md">
                                    {{ settings.translations.cookies_warning.value }}
                                </div>
                                <LibraryButton class="shadow-none mt-5" @click="acceptCookies">
                                    Okay
                                </LibraryButton>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <Transition>
            <div
                v-if="!stageStore.dataIsReady"
                class="fixed top-0 left-0 w-full h-full bg-white z-[20]">
                <div class="w-full h-full flex items-center justify-center">
                    <div role="status">
                        <svg aria-hidden="true" class="w-10 h-10 mr-2 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor"/>
                            <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentFill"/>
                        </svg>
                        <span class="sr-only">Loading...</span>
                    </div>
                </div>
            </div>
        </Transition>
        <div v-if="! isOrientationLandscape">
            <div class="fixed top-0 left-0 w-full h-full bg-white z-[20] flex justify-center items-center">
                <img class="w-[150px]" src="/images/icons/rotate-smartphone.png" alt="">
            </div>
        </div>
    </div>
</template>

<style scoped>
.v-enter-active,
.v-leave-active {
    transition: opacity 2s ease;
}

.v-enter-from,
.v-leave-to {
    opacity: 0;
}
</style>